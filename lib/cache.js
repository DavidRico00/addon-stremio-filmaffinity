...
function getListCache(userId, listId) {
    const key = getListCacheKey(userId, listId);
    const entry = listCache[key];
    if (!entry) return null;

    const maxAge = (parseInt(process.env.CACHE_HOURS) || 1) * 60 * 60 * 1000;
...
module.exports = { init, getImdbId, setImdbId, getListCache, setListCache, clearListCache };
