...
function curlGet(url) {
    try {
        return execSync(
            `curl -s -L --max-time 10 -H "User-Agent: ${USER_AGENT}" "${url}"`,
...
function parseEnglishTitle(html) {
    try {
        ...
        if (!enTitle) {
            const ogTitle = $('meta[property="og:title"]').attr('content') || '';
            if (ogTitle) enTitle = ogTitle.replace(/\s*\(\d{4}\)\s*$/, '').replace(/ - Filmaffinity$/i, '').trim();
        }
        return enTitle || null;
    } catch { return null; }
}

async function resolveWithHtml(faId, title, year, type, esHtml, enHtml) {
    const cached = cache.getImdbId(faId);
    ...
    if (esHtml) {
        const origTitle = parseOriginalTitle(esHtml);
        if (origTitle && origTitle.toLowerCase() !== title.toLowerCase()) {
            console.log(`[IMDb] Trying original title: "${origTitle}" for "${title}"`);
            result = await tryImdbSuggestions(origTitle, year, type);
            if (!result) {
                const simp = simplifyTitle(origTitle);
                if (simp !== origTitle) result = await tryImdbSuggestions(simp, year, type);
            }
        }
    }

    if (!result && enHtml) {
        const enTitle = parseEnglishTitle(enHtml);
        if (enTitle && enTitle.toLowerCase() !== title.toLowerCase()) {
            console.log(`[IMDb] Trying English title: "${enTitle}" for "${title}"`);
            result = await tryImdbSuggestions(enTitle, year, type);
            if (!result) {
                const simp = simplifyTitle(enTitle);
                if (simp !== enTitle) result = await tryImdbSuggestions(simp, year, type);
            }
        }
    }
...
module.exports = { resolveImdbId, resolveAll, resolveWithHtml };
