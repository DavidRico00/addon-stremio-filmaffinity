...
async function resolveImdbId(faId, title, year, type) {
    ...
    try {
        ...
        let imdbId = await tryImdbSuggestions(title, year, type);

        // Strategy 2: Simplified Spanish title (remove subtitle)
        if (!imdbId) {
            const simplified = simplifyTitle(title);
            if (simplified !== title) {
                imdbId = await tryImdbSuggestions(simplified, year, type);
            }
        }
        ...
        if (!imdbId) {
            ...
            if (origTitle && origTitle.toLowerCase() !== title.toLowerCase()) {
                ...
                imdbId = await tryImdbSuggestions(origTitle, year, type);

                if (!imdbId) {
                    const simpOrig = simplifyTitle(origTitle);
                    if (simpOrig !== origTitle) {
                        imdbId = await tryImdbSuggestions(simpOrig, year, type);
                    }
                }
            }
        }
        ...
        if (!imdbId) {
            ...
            if (enTitle && enTitle.toLowerCase() !== title.toLowerCase()) {
                ...
                imdbId = await tryImdbSuggestions(enTitle, year, type);

                if (!imdbId) {
                    const simpEn = simplifyTitle(enTitle);
                    if (simpEn !== enTitle) {
                        imdbId = await tryImdbSuggestions(simpEn, year, type);
                    }
                }
            }
        }
        ...
        if (imdbId) {
            cache.setImdbId(faId, imdbId);
            console.log(`[IMDb] Resolved: "${title}" (${year}) → ${imdbId}`);
            return imdbId;
        }
    } catch (err) {
    ...
    try {
        ...
        for (const item of candidates) {
            ...
            if (type === 'series') {
                ...
                else if (itemType.includes('movie') || itemType.includes('feature')) score -= 2;
            } else {
                if (itemType.includes('movie') || itemType.includes('feature')) score += 3;
                else if (itemType.includes('tvseries') || itemType.includes('tvmini')) score -= 2;
            }
...
async function resolveAll(items) {
    ...
    for (const item of items) {
        const imdbId = await resolveImdbId(item.faId, item.title, item.year, item.type);
        if (imdbId) {
            resolved.push({ ...item, imdbId });
        }
    }
    return resolved;
}
