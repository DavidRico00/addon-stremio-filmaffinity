...
function fetchPage(url) {
    try {
        return execSync(
            `curl -s -L --max-time 15 -H "User-Agent: ${USER_AGENT}" -H "Accept: text/html,application/xhtml+xml" -H "Accept-Language: es-ES,es;q=0.9" "${url}"`,
            { encoding: 'utf8', timeout: 20000 }
        );
    } catch (err) {
        console.error(`[Scraper] curl failed: ${err.message}`);
        return null;
    }
}
...
async function scrapeList(userId, listId) {
    ...
    while (hasMore) {
        ...
        if (!html || !html.includes('<')) {
            console.error(`[Scraper] Failed to fetch userId=${userId}, listId=${listId}, page=${page}`);
            if (page === 1) throw new Error('Filmaffinity blocked the request');
            break;
        }
...
module.exports = { scrapeList, parseHtml };
